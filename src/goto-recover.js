 ...
 * goto-recover
 *
 * Google Search serves signed-out users result links of the form
 * /goto?url=<opaque token> instead of the destination URL. The token cannot be
 * decoded, but the destination is still shipped inside the page. This script
 * reads it back and rewrites the links in place. No network requests, no UI.
 *
 * Where the destination lives:
 *   - inline state  `var m={...}` in a <script>: one record per result. The
 *     "about this result" link inside it carries a `req=` base64url protobuf
 *     whose field 1 is the destination URL.
 *   - deferred payloads in HTML comments `<!--MARKER|||[...]-->`: AI Overview,
 *     People also ask, related results. The full URL sits in the same record.
 *   - YouTube key moments: only the video record exists; the chapter link is
 *     the video URL plus &t=<data-time>.
 *
 * Pairing rule: a token and a URL belong together when they share a data node
 * that contains no other token. No proximity scoring, no DOM crawling.
 *
 * Run it again to re-apply (e.g. after expanding People also ask).
 */
(() => {
  'use strict';

  const NS = '__gotoRecover';
  // Options: { domainFallback: false }. When true, links whose record only
  // exposes the domain are sent to the site root. Off by default: that is not
  // the page Google would send you to.
  const settings = Object.assign({ domainFallback: false }, window[NS + 'Options'] || {});
  if (window[NS]) { window[NS].run(settings); return; }
  ...
  // Confidence, highest first. Links are only rewritten from "derived" upwards.
  ...
  // Field 1 (tag 0x0a, length-delimited) of a base64url protobuf.
  function protoField1(b64) {
    try {
      const bin = atob(b64.replace(/-/g, '+').replace(/_/g, '/'));
      if (bin.charCodeAt(0) !== 0x0a) return null;
      let len = 0, shift = 0, i = 1, byte;
      do { byte = bin.charCodeAt(i++); len |= (byte & 0x7f) << shift; shift += 7; } while (byte & 0x80);
      return new TextDecoder().decode(Uint8Array.from(bin.slice(i, i + len), (c) => c.charCodeAt(0)));
    } catch { return null; }
  }

  // A bare domain root found as a plain string is usually a source label, not a
  // destination, so it is demoted to "domain". A root coming from the `req`
  // protobuf is the real destination (home pages do rank) and keeps its rank.
  ...
  // What a single string contributes: a token, destination candidates, or a title.
  ...
  const resolved = new Map(); // token -> { url, kind, host, title }
  ...
  // Post-order traversal. Every node aggregates the tokens, candidate URLs and
  // titles found below it; a node holding exactly one token is that result's record.
  function walk(value) {
  ...
  // Records that only carry a domain plus a title (related results, some AI
  // Overview sources) borrow the URL of a fully resolved record with the same
  // host and title.
  ...
  // YouTube key moments are rendered from an HTML template, not from data.
  // Chapter = parent video URL + t=<data-time>.
  function deriveKeyMoments() {
    for (const a of document.querySelectorAll('a[data-time][href*="/goto?url="]')) {
      ...
      for (let el = a.parentElement; el && el !== document.body; el = el.parentElement) {
        const parent = [...el.querySelectorAll('a[href*="/goto?url="]:not([data-time]), a[data-goto-token]:not([data-time])')]
          .map((x) => resolved.get(tokenOf(x)))
          .find((r) => r && RANK[r.kind] >= RANK.embedded);
        if (!parent) continue;
        const u = new URL(parent.url);
        u.searchParams.set('t', a.getAttribute('data-time'));
        resolved.set(token, { url: u.href, kind: 'derived', host: u.hostname, title: null });
        break;
      }
    }
  }

  // Some AI Overview source chips carry their link-preview payload in the HTML
  // comment that directly follows the anchor:
  //   <a href="/goto?url=…">Source</a><!--MARKER|||[["https://example.com/page", …
  // The payload has no token, so it is paired by DOM adjacency instead.
  ...
  function pairAdjacentComments() {
    // Longest visible label per token, so adjacent matches can take part in the title inference.
    ...
    for (const a of anchors) {
      ...
      resolved.set(token, { url: hit.url, kind: 'adjacent', host, title: cur?.title || labels.get(token) || null });
    }
  }
  ...
  function run(options) {
    ...
    const left = { ads: 0, domainOnly: 0, noData: 0 };
    for (const a of document.querySelectorAll('a[href*="/goto?url="]')) {
      const r = resolved.get(tokenOf(a));
      if (a.closest('[data-text-ad], [data-pcu], [data-rw]')) left.ads++;
      else if (r) left.domainOnly++;
      else left.noData++;
    }
    ...
    console.info(
      `goto-recover: ${rewritten} links rewritten` + (toRoot ? ` (${toRoot} of them to the site root only)` : '') +
      `, ${pending} left (${left.ads} ads, ${left.domainOnly} with only the domain in the page, ${left.noData} with no data in the page) ` +
      `in ${Math.round(performance.now() - t0)} ms`
    );
    return { rewritten, toRoot, pending, left };
  }

  // Re-run when Google injects new result blocks (People also ask, lazy carousels).
  let timer = null;
  const observer = new MutationObserver(() => {
    clearTimeout(timer);
    timer = setTimeout(() => { if (document.querySelector('a[href*="/goto?url="]')) run(); }, 400);
  });
  observer.observe(document.body, { childList: true, subtree: true });

  window[NS] = { run, stop: () => observer.disconnect(), resolved };
  run();
})();
